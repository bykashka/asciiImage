...
def get_color(im_obj):
    ...
    ascii_pic = open(os.path.join(path_to_save_ACSII_image, 'ACSII_' + image_JPEG_name[:-4]+'.html'), 'w')
...
#    print x, y
    ...
    for i in xrange(y):
        for j in xrange(x):
            ...
            if 0 <= color_px <= 16:
                simbol = '<FONT COLOR=#%s%s%s>#</FONT COLOR>' % (r, g, b)
            elif 17 <= color_px <= 32:
                simbol = '<FONT COLOR=#%s%s%s>M</FONT COLOR>' % (r, g, b)
            elif 33 <= color_px <= 48:
                simbol = '<FONT COLOR=#%s%s%s>@</FONT COLOR>' % (r, g, b)
            elif 49 <= color_px <= 64:
                simbol = '<FONT COLOR=#%s%s%s>H</FONT COLOR>' % (r, g, b)
            elif 65 <= color_px <= 80:
                simbol = '<FONT COLOR=#%s%s%s>X</FONT COLOR>' % (r, g, b)
            elif 81 <= color_px <= 96:
                simbol = '<FONT COLOR=#%s%s%s>$</FONT COLOR>' % (r, g, b)
            elif 97 <= color_px <= 112:
                simbol = '<FONT COLOR=#%s%s%s>'  % (r, g, b) + '%</FONT COLOR>'
...
#im_gray_scale.save(os.path.join(path_to_save_ACSII_image, 'ACSII_' + image_JPEG_name), 'JPEG')
